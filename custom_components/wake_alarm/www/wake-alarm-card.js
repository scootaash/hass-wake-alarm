...
`,gt=["mon","tue","wed","thu","fri","sat","sun"],vt=["length_min","start_kelvin","target_kelvin","max_brightness_pct","volume","snooze_min","steps_per_min","music_fade_sec","auto_dismiss_min"],_t=["test_light_ramp","cancel_ramp","test_music","dismiss","snooze"],ft=["next_alarm","state","media_selection"];class bt extends Error{}let yt=class extends nt{constructor(){super(...arguments),this._toggleEnabled=()=>{this.hass&&this.related&&this.hass.callService("switch","toggle",{entity_id:this.related.enabled})},this._snooze=()=>{this.hass&&this.related&&this.hass.callService("button","press",{entity_id:this.related.buttons.snooze})},this._dismiss=()=>{this.hass&&this.related&&this.hass.callService("button","press",{entity_id:this.related.buttons.dismiss})},this._goSettings=()=>{this.dispatchEvent(new CustomEvent("navigate-settings",{bubbles:!0,composed:!0}))}}connectedCallback(){super.connectedCallback(),this._tickInterval=window.setInterval(()=>this.requestUpdate(),1e3)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._tickInterval&&(window.clearInterval(this._tickInterval),this._tickInterval=void 0)}render(){if(!this.hass||!this.related)return B``;const t=this.related,e=this.hass.states[t.enabled],i=this.hass.states[t.sensors.state],s=this.hass.states[t.active],o=this.hass.states[t.alarmTime],r=this.hass.states[t.sensors.next_alarm],a="on"===e?.state,n=i?.state??"idle",c="on"===s?.state,l=xt(o?.state),d=$t[n]??$t.idle,h=a?function(t){switch(t){case"ramping":return"Ramping";case"playing":return"Playing";case"snoozing":return"Snoozing";default:return"On"}}(n):"Off",p=i?.attributes?.snooze_until,u="snoozing"===n&&p?function(t){const e=new Date(t).getTime();if(Number.isNaN(e))return t;const i=Math.max(0,Math.round((e-Date.now())/1e3)),s=Math.floor(i/60);return`${s}:${kt(i%60)}`}(p):null,m=u?`Music in ${u}`:r?.state&&"unknown"!==r.state?function(t){const e=new Date(t);if(Number.isNaN(e.getTime()))return t;const i={weekday:"short",hour:"2-digit",minute:"2-digit",hour12:!1};return new Intl.DateTimeFormat(void 0,i).format(e)}(r.state):"No upcoming alarm";return B`
      <ha-card>
        <div class="header">
        ...
        <div class="mode-tile mode-${a?n:"off"}" @click=${this._toggleEnabled}>
          ...
          <div class="mode-text">
            ...
            <div class="mode-next">${a?m:"Tap to enable"}</div>
          </div>
        </div>
  ...
  `,t([pt({attribute:!1})],At.prototype,"hass",void 0),t([pt()],At.prototype,"entityId",void 0),t([ut()],At.prototype,"_children",void 0),t([ut()],At.prototype,"_path",void 0),t([ut()],At.prototype,"_loading",void 0),t([ut()],At.prototype,"_error",void 0),At=t([lt("wake-alarm-media-browser")],At);const Et=[{key:"snooze_min",label:"Snooze (min)",description:"How long the snooze pause lasts before music resumes.",min:1,max:30,step:1},{key:"length_min",label:"Length (min)",description:"Total minutes the lights ramp up before the alarm time.",min:1,max:120,step:1},{key:"start_kelvin",label:"Start K",description:"Warm colour temperature at the beginning of the ramp.",min:1500,max:6500,step:50},{key:"target_kelvin",label:"Target K",description:"Cool colour temperature reached at the alarm time.",min:1500,max:6500,step:50},{key:"max_brightness_pct",label:"Max % Brightness",description:"Peak brightness reached at the alarm time.",min:1,max:100,step:1},{key:"volume",label:"Alarm Volume (0–1)",description:"Final volume the music fades up to. Defaults capped low to prevent accidents.",min:0,max:1,step:.01},{key:"music_fade_sec",label:"Music fade (s)",description:"How long the volume takes to fade from 0 to the target volume.",min:0,max:300,step:5},{key:"auto_dismiss_min",label:"Auto-dismiss (min)",description:"Stop everything automatically after this long. 0 disables.",min:0,max:120,step:1}];let St=class extends nt{constructor(){super(...arguments),this._showMediaPicker=!1,this._openMediaPicker=()=>{this._showMediaPicker=!0},this._closeMediaPicker=()=>{this._showMediaPicker=!1},this._onMediaPicked=t=>{if(!this.hass||!this.related)return;const e=t.detail?.item;e&&(this.hass.callService("wake_alarm","set_media",{media_content_id:e.media_content_id,media_content_type:e.media_content_type,title:e.title??e.media_content_id,thumbnail:e.thumbnail},{entity_id:this.related.enabled}),this._closeMediaPicker())},this._openOptionsFlow=()=>{this.related&&(history.pushState(null,"","/config/integrations/integration/wake_alarm"),window.dispatchEvent(new Event("location-changed")))},this._goBack=()=>{this.dispatchEvent(new CustomEvent("navigate-back",{bubbles:!0,composed:!0}))}}shouldUpdate(t){return t.has("hass")||t.has("related")||t.has("_showMediaPicker")}render(){if(!this.hass||!this.related)return B``;const t=this.related,e=this.hass.states[t.sensors.state]?.state??"idle",i=this.hass.states[t.sensors.media_selection],s=i?.state??"none",o="none"!==s,r=i?.attributes?.thumbnail,a=this.hass.states[t.sensors.next_alarm],n=a?.attributes?.light_entities??[],c=a?.attributes?.media_player_entities??[],l=a?.attributes?.person_entity;return B`
      <ha-card>
        <div class="header">
        ...
        <div class="section media">
          ...
          <div class="media-row" @click=${this._openMediaPicker}>
            ${o?B`
                  ${r?B`<img src=${r} alt="" class="thumb" />`:B`<div class="thumb thumb-placeholder">
    ...
    `}_renderSlider(t){if(!this.hass||!this.related)return B``;const e=this.related.numbers[t.key],i=this.hass.states[e]?.state,s=i&&!["unknown","unavailable"].includes(i)?Number(i):t.min,o=t.step>=1?s.toFixed(0):s.toFixed(2);return B`
    ...
    .error { color: var(--error-color, rgb(255, 82, 82)); }
  `,t([pt({attribute:!1})],Ct.prototype,"hass",void 0),t([ut()],Ct.prototype,"_config",void 0),t([ut()],Ct.prototype,"_enabledSwitches",void 0),t([ut()],Ct.prototype,"_loadError",void 0),Ct=t([lt("wake-alarm-card-editor")],Ct);let Pt=class extends nt{constructor(){super(...arguments),this._view="main",this._goMain=()=>{this._view="main"},this._goSettings=()=>{this._view="settings"}}setConfig(t){if(!t?.entity)throw new Error("wake-alarm-card: `entity` is required.");if(!t.entity.startsWith("switch."))throw new Error("wake-alarm-card: `entity` must be a switch (the wake_alarm enabled switch).");this._config=t,this._related=void 0,this._resolveError=void 0}getCardSize(){return 6}static getConfigElement(){return document.createElement("wake-alarm-card-editor")}static getStubConfig(){return{type:"custom:wake-alarm-card",entity:""}}willUpdate(t){this.hass&&this._config&&!this._related&&!this._resolveError&&this._resolveRelated()}async _resolveRelated(){if(this.hass&&this._config)try{const t=await this.hass.callWS({type:"config/entity_registry/list"});this._related=function(t,e){const i=e.find(e=>e.entity_id===t);if(!i)throw new bt(`Entity ${t} is not in the entity registry.`);if("wake_alarm"!==i.platform)throw new bt(`Entity ${t} is not a wake_alarm entity (platform=${i.platform}).`);const s=i.config_entry_id;if(!s)throw new bt(`Entity ${t} has no config_entry_id.`);const o={},r=`${s}_`;for(const t of e){if(t.config_entry_id!==s)continue;if(!t.unique_id||!t.unique_id.startsWith(r))continue;const e=t.unique_id.slice(r.length);o[e]=t.entity_id}const a=t=>{const e=o[t];if(!e)throw new bt(`Wake Alarm entity for "${t}" is missing from the registry. Make sure the integration is fully loaded.`);return e},n={};for(const t of gt)n[t]=a(t);const c={};for(const t of vt)c[t]=a(t);const l={};for(const t of _t)l[t]=a(t);const d={};for(const t of ft)d[t]=a(t);return{configEntryId:s,enabled:a("enabled"),active:a("active"),alarmTime:a("alarm_time"),days:n,numbers:c,buttons:l,sensors:d}}(this._config.entity,t)}catch(t){this._resolveError=t instanceof bt?t.message:`Could not resolve wake_alarm entities: ${t}`}}render(){return this._config?this._resolveError?B`<ha-card><div class="error">${this._resolveError}</div></ha-card>`:this._related&&this.hass?"settings"===this._view?B`
